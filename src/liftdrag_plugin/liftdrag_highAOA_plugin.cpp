...
#include "liftdrag_plugin/liftdrag_highAOA_plugin.h"

#include "Force.pb.h"

using namespace gazebo;

GZ_REGISTER_MODEL_PLUGIN(LiftDragHighAOAPlugin)

/////////////////////////////////////////////////
LiftDragHighAOAPlugin::LiftDragHighAOAPlugin() : cl0(0.0), cd0(0.01), cm0(0.0), cla(1.0), cda(0.01), cma(0.0), rho(1.2041)
{
...
/////////////////////////////////////////////////
LiftDragHighAOAPlugin::~LiftDragHighAOAPlugin()
{
}

/////////////////////////////////////////////////
void LiftDragHighAOAPlugin::Load(physics::ModelPtr _model,
                     sdf::ElementPtr _sdf)
{
  GZ_ASSERT(_model, "LiftDragHighAOAPlugin _model pointer is NULL");
  GZ_ASSERT(_sdf, "LiftDragHighAOAPlugin _sdf pointer is NULL");
  this->model = _model;
  this->sdf = _sdf;

  this->world = this->model->GetWorld();
  GZ_ASSERT(this->world, "LiftDragHighAOAPlugin world pointer is NULL");
...
#endif
  GZ_ASSERT(this->physics, "LiftDragHighAOAPlugin physics pointer is NULL");
  ...
  if (_sdf->HasElement("m"))
    this->M = _sdf->Get<double>("m");
  ...
  if (_sdf->HasElement("link_name"))
  {
    ...
    if (!this->link)
    {
      gzerr << "Link with name[" << linkName << "] not found. "
        << "The LiftDragHighAOAPlugin will not generate forces\n";
    }
    else
    {
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          boost::bind(&LiftDragHighAOAPlugin::OnUpdate, this));
    }
  }
  ...
  if (_sdf->HasElement("windSubTopic")){
    ...
    wind_sub_ = node_handle_->Subscribe("~/" + wind_sub_topic_, &LiftDragHighAOAPlugin::WindVelocityCallback, this);
  }
...
/////////////////////////////////////////////////
void LiftDragHighAOAPlugin::OnUpdate()
{
  GZ_ASSERT(this->link, "Link was NULL");
  ...
  double cl, cl_sa, cl_ha, cl_delta = 0.0;
  double delta, ele1, ele2;
  cl_sa = this->cl0 + this->cla * this->alpha * cosSweepAngle;
  cl_ha = sin(2 * this->alpha) * cosSweepAngle;
  ele1 = exp(-this->M * (this->alpha - this->alphaStall));
  ele2 = exp( this->M * (this->alpha + this->alphaStall));
  delta = (1+ele1+ele2)/(1+ele1)/(1+ele2);
...
#endif
    cl_delta = this->controlJointRadToCL * controlAngle;
    /// \TODO: also change cd
  }
  cl = (1 - delta) * (cl_sa + cl_delta) + delta * cl_ha;

  // compute lift force at cp
  ignition::math::Vector3d lift = cl * q * this->area * liftI;

  // compute cd at cp, check for stall, correct for sweep
  double cd, cd_sa, cd_ha;
  cd_sa = this->cd0 + (cl_sa * cl_sa)/M_PI/0.9/this->AR * cosSweepAngle;
  cd_ha = 2 * sin(this->alpha) * sin(this->alpha) * cosSweepAngle;
  cd = (1 - delta) * cd_sa + delta * cd_ha;
...
void LiftDragHighAOAPlugin::WindVelocityCallback(const boost::shared_ptr<const physics_msgs::msgs::Wind> &msg) {
